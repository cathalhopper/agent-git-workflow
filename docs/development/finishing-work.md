...
|---|---|---|
| `git will not read a repository here` | `git rev-parse --git-dir` failed, and git's own line above the stop says which reason: no repository at this path, or a checkout git refuses — `detected dubious ownership`, which a container, a sandbox and a network or cloud-synced path all produce | Act on what git said. Where it names dubious ownership, run the `git config --global --add safe.directory <path>` line git prints with it |
| `git fetch could not reach origin` | The fetch failed, and `origin` does not answer `git ls-remote` either | Fix the connection and re-run. Every check reads remote-tracking refs, and one against a stale remote answers with an authority it has not earned |
| `git fetch failed on <remote>, not on origin` | `git fetch --all` exits non-zero when any remote fails, and the one that failed is another remote in this checkout — a fork keeps its `upstream` here | Fix that remote's URL, or remove it with `git remote remove <name>`, then re-run. `origin` answers |
| `git fetch --all --prune failed` | The fetch failed while `origin` and every other remote answer a read, so the connection is not the cause | Act on what git said above the stop, then re-run |
...
| `gh could not read your GitHub login` | `gh api user` returns no login: a token without the `read:user` scope, a lapsed SSO authorisation, or an error from GitHub. The author check compares that login against the pull request's author, and the script never guesses it | Act on what gh said, then `gh auth status`. Authenticate again where that is the cause: [`setting-up.md`](setting-up.md) §3.3 |
...
| `no commit in this repository is named <sha>` | The commit the cleanup confirms against resolves to nothing: a truncated or mistyped `--sha` (`-Sha` in PowerShell), or a commit this repository has never held | Read the SHA off `git log --oneline origin/<base>` and re-run. Nothing is deleted until the log confirms the work landed |
| `gh could not read the merge commit for pull request #<n>` (after the merge) | The squash merge went through, and the `gh pr view` that reads the merge commit back failed for the reason it names above the stop | Act on what gh said, then read the squashed commit off `git log --oneline origin/<base>` and finish with `--cleanup --branch <branch> --sha <commit>` (`-Cleanup -Branch -Sha` in PowerShell). The merge is done, as `Already done in this run` lists |
| `the pull request merged but GitHub did not report a merge commit` (after the merge) | The squash merge went through, and `gh` answered with no merge commit in it | Read the squashed commit off `git log --oneline origin/<base>` and finish it with `--cleanup --branch <branch> --sha <commit>`. Nothing is deleted until the log confirms the work landed |
| `<base> could not be fast-forwarded, and it has not drifted` (after the merge) | Local `<base>` is still on `origin/<base>`'s history, so the fast-forward failed on the working tree, for the reason in git's message above the note — a file it cannot write, held open by a process or read-only | Close whatever holds that file, or clear its read-only flag, then `git merge --ff-only origin/<base>` from the primary checkout |
